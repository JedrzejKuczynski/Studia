...
in vec2 iTexCoord1; //wspolrzedne teksturowania
...
void main(void){
    ...
    vec4 kd = texture(textureMap1, iTexCoord1);
    ...
    float rv1 = pow(max(dot(r1,v),0),100); //Kosinus kata pomiedzy wektorami do obserwatora i odbitym, podniesiony do wykladnika Phonga
    float rv2 = pow(max(dot(r2,v),0),100);
